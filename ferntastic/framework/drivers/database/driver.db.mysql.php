...
if (!defined('ABSPATH')) die("Must be loaded as an extension");

/**
 * MYSQL Exception class.
 */
 
class MySQLError extends NoLogError {
...
class MySQL implements DatabaseDriver {
	...
	public $last_sql = '';
	private static $db_host, $db_user, $db_password, $db_name;
	
	function __construct( $db_host, $db_user, $db_password, $db_name ) {
	
		if (false) {
			$db_object = @mysqli_connect( self::DB_HOST, self::DB_USER, self::DB_PASSWORD, self::DB_NAME );
			$this->db_extensions['91f'] = $db_object;
	...
	function query( $sql ) {
		...
		if (!isset($this->db_object) or !$this->db_object) {
			throw new MySQLError('nodbobj');
		} else {
			
			$q = mysqli_query( $this->db_object, $sql );
			$this->last_sql = $sql;
			if (!$q) throw new MySQLError( ( $e = mysqli_error( $this->db_object ) ) ? $e : null );
			else {
				$this->last_query = $q;
				return $q;
			}
	...
	public static function lookup( $table, $id, $stringkey, $lookupkey = "id" ) {
		...
		else: false; /*throw new MySQLError(0);*/ endif;
		return $ret;
...
class MySQLBackup {
	...
	function __construct() {
		...
		try {
			...
			if (!$working) {
				throw new MySQLError("Can't write to `fBackup`. Does it exist?");
			}
