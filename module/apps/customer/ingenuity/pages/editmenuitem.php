...
//check if there is an ID to use
	...
	$f = fQuery('patsys_menu_items', 'id[x=?],*', e($id));
	...
	if ($f->count > 0):
		...
		if ($t->imglink != "") {
			$img = "http://wp.patsys.testing.91ferns.com/files/".basename($t->imglink)."?w=200";
			$IMGS = '<ul class="qq-fake">';
			$IMGS .= '<li class="qq-upload-success" data-id="'.$data->id.'"><span class="qq-upload-file"><a class="ferns" style="text-decoration: none;" href="'.$img.'">'.basename($t->imglink).'</a></span></li>';
			$IMGS .= '<li>Upload a new file to replace this image.</li>';
			$IMGS .= '</ul>';
		} else $IMGS = '';
		
		$html = '<div id="fupload-div"></div><span id="fstatus-text"></span>';
		$catname = lookup('patsys_menu_categories', $t->category, "[displayname]", 'id');
		echo new WebForm('ItemEntry',
			array(
			'ItemID' => array('type' => "hidden", 'value' => $t->id ),
			'ItemName' => array('type' => 'text', 'label' => "Item Name", 'value' => $t->name),
			'ItemPrice' => array('type' => 'text', 'label' => "Price", 'value' => number_format($t->price, 2)),
			'Category' => array('type' => 'text', 'label' => "Category", 'value' => $catname),
			"ItemDescription" => array('type' => 'textarea', 'label' => "Description", 'value' => $t->description),
			'do' => array('type' => 'hidden', 'value' => 'edit_menu_item'),
			 'Attch' => array(
				'type' => 'html',
				'value' => $IMGS.$html,
				'label' => "Item Image"
			),
			array('type' => "submit", 'value' => "Edit Item")
			),
			'/do-new.php'
		);
