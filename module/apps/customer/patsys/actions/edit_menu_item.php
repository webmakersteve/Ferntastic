...
if (!$name or strlen($name) < 3) {
	...
	$sql = "UPDATE `patsys_menu_items` SET ";
	
	if ($daUPLOAD) $sql .= sprintf( " price = %.2f, description = '%s', name = '%s', category = %d, imglink = '%s' ", 
					floatval( $price ),
					e($desc),
					e($name), 
					$catID,
					e($daUPLOAD) );
	else $sql .= sprintf( " price = %.2f, description = '%s', name = '%s', category = %d ", 
					floatval( $price ),
					e($desc),
					e($name), 
					$catID );
	
	$sql .= " WHERE id = ".intval($_POST['ItemID']). " LIMIT 1";
					
	query($sql);
					
	
	$r = "Successfully edited that menu item.";
	$s = "ok";
